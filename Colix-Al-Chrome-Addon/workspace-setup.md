...
The Teams page uses `api/paypal.php` for recurring PayPal Subscriptions. It supports monthly billing and annual billing at 5% below twelve monthly payments. Apply `supabase/paypal-subscriptions.sql` before deploying the endpoint. Configure `PAYPAL_ENV=sandbox` or `PAYPAL_ENV=production`, `PAYPAL_CLIENT_ID`, `PAYPAL_CLIENT_SECRET`, `PAYPAL_CURRENCY`, one `PAYPAL_PLAN_<PLAN_CODE>` and one `PAYPAL_PLAN_<PLAN_CODE>_ANNUAL` per paid plan, and `PAYPAL_WEBHOOK_ID`; annual PayPal plans must be active yearly plans priced at `monthly_price * 12 * 0.95`. Never place secrets in the extension or source control. Deploy `api/paypal-webhook.php` as the PayPal webhook URL and subscribe it to activation, suspension, cancellation, expiration, and completed sale events.
