...
The Teams page uses `api/paypal.php` for recurring PayPal Subscriptions. It creates a server-side subscription from the configured monthly PayPal plan, sends the customer to PayPal approval, verifies the approved subscription on return, and stores the provider subscription ID. Apply `supabase/paypal-subscriptions.sql` before deploying the endpoint. Configure `PAYPAL_ENV=sandbox` or `PAYPAL_ENV=production`, `PAYPAL_CLIENT_ID`, `PAYPAL_CLIENT_SECRET`, `PAYPAL_CURRENCY`, one `PAYPAL_PLAN_<PLAN_CODE>` per paid plan, and `PAYPAL_WEBHOOK_ID`; never place secrets in the extension or source control. Deploy `api/paypal-webhook.php` as the PayPal webhook URL and subscribe it to activation, suspension, cancellation, expiration, and completed sale events.

The PayPal operations page is `api/paypal-admin.html`. Set a long random `PAYPAL_ADMIN_TOKEN` in the server `.env`, open the page over HTTPS, and enter that token. It can list PayPal plans and webhooks, create monthly plans from paid active rows in `workspace_plan_catalog`, create the webhook with the required events, and inspect a subscription by ID. The endpoint is `api/paypal-admin.php`; the PayPal client secret is used only on the server. Use matching credentials and environment: `PAYPAL_ENV=production` requires production credentials, while any other value uses PayPal Sandbox.
